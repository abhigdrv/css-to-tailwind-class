...
const Output = ({ settings, cssTree, editorErrors }) => {
    const handleFinalResult = (val) => {
        var getOne = localStorage.getItem('finalTailwindCss');
        if(getOne){
            var res = JSON.parse(getOne);
            if(res.findIndex(x => x.identifier == 'xample1') != -1){
                res = [];
                localStorage.setItem('finalTailwindCss', JSON.stringify(res));
            }
            res.push(val);
            localStorage.setItem('finalTailwindCss', JSON.stringify(res));
        }else{
    ...
    return (
        <div className="flex-grow h-full w-full p-2 border-box overflow-y-auto">
            <div className="flex tracking-wide text-teal-900 text-lg items-center uppercase font-bold mb-2">
                <svg
                    fill="none"
                    stroke="currentColor"
                    strokeLinecap="round"
